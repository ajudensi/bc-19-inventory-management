...
router.get('/add-assets', (req, res) => {
  if ((req.session) && ((req.session.role === 'super') || (req.session.role === 'admin'))) {
...
router.get('/assigned', (req, res) => {
  if (!(req.session) || (req.session.role !== 'super' || req.session.role !== 'admmin')) {
    res.redirect('/');
  } else {
    fireApp.auth().signInWithEmailAndPassword(req.session.email, req.session.password).then(() => {
...
router.post('/report', (req, res) => {
  fireApp.auth().signInWithEmailAndPassword(req.session.email, req.session.password).then(() => {
    ...
    fireUserRef.child(`${uid}`).child('assignedItems').child(`${assetId}`).update({
      ...
      if (!error) {
        iManagerMailer('I-Manager', 'a.joshuaudensi@gmail.com', 'Case of lost item', `Asset with Andela code ${assetId}, assigned to ${username} has been reported missing`);
        res.redirect('/assets');
      }
    });
  });
...
router.post('/found', (req, res) => {
  fireApp.auth().signInWithEmailAndPassword(req.session.email, req.session.password).then(() => {
    ...
    fireUserRef.child(`${uid}`).child('assignedItems').child(`${assetId}`).update({
      ...
      if (!error) {
        iManagerMailer('I-Manager', 'a.joshuaudensi@gmail.com', 'Case of lost but found item', `Lost asset with Andela code ${assetId}, assigned to ${username} has been reported as found`)
        res.redirect('/assets');
      }
    });
  });
